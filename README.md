...
| `media/` | Logos, hero video/stills, About photo. |
...
Hero → About us (Vision / Mission / Core values) → Our Process → Our Solutions → Services → Why Envex → Contact → Footer.
...
- Motion: scroll reveals via IntersectionObserver with a staggered inner rise on cards; nav tints past 40px; anchor scrolling is JS-eased with a 96px offset. All respect `prefers-reduced-motion` except the anchor easing.
